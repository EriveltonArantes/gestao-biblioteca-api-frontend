...
const apiBaseUrl = "https://gestao-biblioteca-api.onrender.com";
    ...
    <div className="login-screen">
      <div className="login-institucional">
        ...
        <ul className="login-features">
          <li>✓ Controle completo de Livro</li>
          <li>✓ Controle completo de Leitor</li>
          <li>✓ Controle completo de Emprestimo</li>
...
function PainelLivro({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  const itensFiltrados = itens.filter(item => !busca || ["titulo", "autor", "isbn", "editora", "anoPublicacao", "categoria", "exemplares", "disponivel", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  ...
  const carregar = () => fetch(apiBaseUrl + "/api/livros", { headers: authHeaders(token) })
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/livros/" + editId : apiBaseUrl + "/api/livros";
    fetch(url, {
  ...
  const editar = (item) => {
    const f = {};
    ["titulo", "autor", "isbn", "editora", "anoPublicacao", "categoria", "exemplares", "disponivel"].forEach(k => { f[k] = item[k] ?? ""; });
    [].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/livros/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["titulo"] ?? "Livro")}</div>
            
            <div className="item-meta-grid">
            <div className="item-field"><b>autor</b><span>{item["autor"] ?? "—"}</span></div>
            <div className="item-field"><b>isbn</b><span>{item["isbn"] ?? "—"}</span></div>
            <div className="item-field"><b>editora</b><span>{item["editora"] ?? "—"}</span></div>
            <div className="item-field"><b>anoPublicacao</b><span>{item["anoPublicacao"] ?? "—"}</span></div>
            <div className="item-field"><b>categoria</b><span>{item["categoria"] ?? "—"}</span></div>
            <div className="item-field"><b>exemplares</b><span>{item["exemplares"] ?? "—"}</span></div>
      ...
      <input type="text" value={form["editora"] ?? ""} onChange={e => setForm({...form, editora: e.target.value})} />
      <label className="field-label">anoPublicacao</label>
      ...
      <label className="field-label">exemplares</label>
      <input type="number" value={form["exemplares"] ?? ""} onChange={e => setForm({...form, exemplares: e.target.value})} />
...
function PainelLeitor({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [livroList, setLivroList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["nome", "cpf", "email", "telefone", "endereco", "tipo", "dataValidade", "ativo", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/leitors", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/livros", { headers: authHeaders(token) }).then(r => r.json()).then(setLivroList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/leitors/" + editId : apiBaseUrl + "/api/leitors";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["nome", "cpf", "email", "telefone", "endereco", "tipo", "dataValidade", "ativo"].forEach(k => { f[k] = item[k] ?? ""; });
    ["livro"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/leitors/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
      ...
      {true && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["nome"] ?? "Leitor")}</div>
            ...
            <div className="item-field"><b>endereco</b><span>{item["endereco"] ?? "—"}</span></div>
            ...
            <div className="item-field"><b>livro</b><span>{item.livroId ? ((livroList.find(o => o.id === item.livroId) || {}).titulo ?? ("#" + item.livroId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Leitor</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
      ...
      <select value={form["livro"] ?? ""} onChange={e => setForm({...form, livro: e.target.value})}>
        <option value="">Selecione...</option>
        {(livroList || []).map(o => (<option key={o.id} value={o.id}>{o.titulo ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelEmprestimo({ token, apiBaseUrl, onPix, role, onLogout }) {
  const [itens, setItens] = React.useState([]);
  const [carregando, setCarregando] = React.useState(true);
  const [form, setForm] = React.useState({});
  const [editId, setEditId] = React.useState(null);
  ...
  const [livroList, setLivroList] = React.useState([]);
  const [leitorList, setLeitorList] = React.useState([]);

  // Paginação real (achado real auditando como cliente exigente: lista sem
  // limite nenhum, "queria ver pelo menos os 10 primeiros"). Front-end só —
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["livroId", "leitorId", "dataRetirada", "dataPrevista", "dataDevolucao", "renovacoes", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);

  const carregar = () => fetch(apiBaseUrl + "/api/emprestimos", { headers: authHeaders(token) })
    .then(r => { if (r.status === 401) { onLogout && onLogout(); return []; } return r.json(); })
    .then(data => { if (Array.isArray(data)) setItens(data); })
    .catch(() => setItens([]))
    .finally(() => setCarregando(false));
  ...
  React.useEffect(() => {
    carregar();
    fetch(apiBaseUrl + "/api/livros", { headers: authHeaders(token) }).then(r => r.json()).then(setLivroList).catch(() => {});
    fetch(apiBaseUrl + "/api/leitors", { headers: authHeaders(token) }).then(r => r.json()).then(setLeitorList).catch(() => {});
  }, [token]);
  ...
  const salvar = (e) => {
    ...
    setErroSalvar("");
    setSalvando(true);
    const url = editId ? apiBaseUrl + "/api/emprestimos/" + editId : apiBaseUrl + "/api/emprestimos";
    fetch(url, {
      method: editId ? "PUT" : "POST",
      headers: { "Content-Type": "application/json", ...authHeaders(token) },
      body: JSON.stringify(montarCorpo()),
  ...
  const editar = (item) => {
    const f = {};
    ["livroId", "leitorId", "dataRetirada", "dataPrevista", "dataDevolucao", "renovacoes", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["livro", "leitor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };

  const apagar = (id) => fetch(apiBaseUrl + "/api/emprestimos/" + id, { method: "DELETE", headers: authHeaders(token) })
    .then(async r => {
      if (r.status === 401) { onLogout && onLogout(); return; }
      if (!r.ok) {
        const corpo = await r.json().catch(() => ({}));
  ...
  return (
    <div>
            ...
            <div className="kanban-coluna" key={coluna}>
              ...
              {itensFiltrados.filter(i => String(i.status ?? "Sem status") === coluna).map(item => (
                <div className="kanban-card" key={item.id}>
                  <div className="kanban-card-titulo">{String(item["status"] ?? "Emprestimo")}</div>
                  <select className="kanban-select" value={item.status ?? ""} onChange={e => {
                    fetch(apiBaseUrl + "/api/emprestimos/" + item.id, {
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-title">{String(item["status"] ?? "Emprestimo")}</div>
            <span className={"status-pill " + corStatus(item.status)}>{String(item.status ?? "")}</span>
            <div className="item-meta-grid">
            <div className="item-field"><b>livroId</b><span>{item["livroId"] ?? "—"}</span></div>
            <div className="item-field"><b>leitorId</b><span>{item["leitorId"] ?? "—"}</span></div>
            <div className="item-field"><b>dataRetirada</b><span>{item["dataRetirada"] ? new Date(item["dataRetirada"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            ...
            <div className="item-field"><b>dataDevolucao</b><span>{item["dataDevolucao"] ? new Date(item["dataDevolucao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>renovacoes</b><span>{item["renovacoes"] ?? "—"}</span></div>
            <div className="item-field"><b>livro</b><span>{item.livroId ? ((livroList.find(o => o.id === item.livroId) || {}).titulo ?? ("#" + item.livroId)) : "—"}</span></div>
            <div className="item-field"><b>leitor</b><span>{item.leitorId ? ((leitorList.find(o => o.id === item.leitorId) || {}).nome ?? ("#" + item.leitorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
              <button className="btn btn-ghost btn-sm" onClick={() => editar(item)}>Editar</button>
              {role === "ADMIN" && <button className="btn btn-danger btn-sm" onClick={() => { if(window.confirm("Confirmar exclusão?")) apagar(item.id); }}>Apagar</button>}
      ...
      {modalAberto && (
        <div className="modal-overlay" onClick={() => setModalAberto(false)}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>{editId ? "Editar" : "Novo"} Emprestimo</h3>
            {erroSalvar && <div className="campo-erro modal-erro">{erroSalvar}</div>}
            <form onSubmit={salvar}>
              <label className="field-label">livroId</label>
      <input type="number" value={form["livroId"] ?? ""} onChange={e => setForm({...form, livroId: e.target.value})} />
      <label className="field-label">leitorId</label>
      <input type="number" value={form["leitorId"] ?? ""} onChange={e => setForm({...form, leitorId: e.target.value})} />
      <label className="field-label">dataRetirada</label>
      <input type="datetime-local" value={form["dataRetirada"] ?? ""} onChange={e => setForm({...form, dataRetirada: e.target.value})} />
      ...
      <label className="field-label">dataDevolucao</label>
      <input type="datetime-local" value={form["dataDevolucao"] ?? ""} onChange={e => setForm({...form, dataDevolucao: e.target.value})} />
      <label className="field-label">renovacoes</label>
      <input type="number" value={form["renovacoes"] ?? ""} onChange={e => setForm({...form, renovacoes: e.target.value})} />
      ...
      <label className="field-label">leitor</label>
      <select value={form["leitor"] ?? ""} onChange={e => setForm({...form, leitor: e.target.value})}>
        <option value="">Selecione...</option>
        {(leitorList || []).map(o => (<option key={o.id} value={o.id}>{o.nome ?? ("#" + o.id)}</option>))}
      </select>
              <div className="modal-actions">
                <button type="submit" className="btn btn-primary" disabled={salvando}>{salvando ? "Salvando..." : "Salvar"}</button>
                <button type="button" className="btn btn-ghost" onClick={() => setModalAberto(false)} disabled={salvando}>Cancelar</button>
...
function PainelReserva({ token, apiBaseUrl, onPix, role, onLogout }) {
  ...
  // não muda o contrato da API (que os outros 9 projetos já usam tal como
  // está), só corta o que já foi carregado em páginas de 10.
  const itensFiltrados = itens.filter(item => !busca || ["livroId", "leitorId", "dataSolicitacao", "dataExpiracao", "status", "id"].some(k => String(item[k] ?? "").toLowerCase().includes(busca.toLowerCase())));
  const totalPaginas = Math.max(1, Math.ceil(itensFiltrados.length / 10));
  const paginaAtual = Math.min(pagina, totalPaginas);
  const itensPagina = itensFiltrados.slice((paginaAtual - 1) * 10, paginaAtual * 10);
  ...
  const editar = (item) => {
    const f = {};
    ["livroId", "leitorId", "dataSolicitacao", "dataExpiracao", "status"].forEach(k => { f[k] = item[k] ?? ""; });
    ["livro", "leitor"].forEach(k => { f[k] = item[k + "Id"] ?? ""; });
    setErroSalvar(""); setForm(f); setEditId(item.id); setModalAberto(true);
  };
  ...
  return (
    <div>
      ...
      {!modoKanban && <><div className="items-grid">
        {itensPagina.map(item => (
          <div className="item-card" key={item.id}>
            ...
            <div className="item-field"><b>dataExpiracao</b><span>{item["dataExpiracao"] ? new Date(item["dataExpiracao"]).toLocaleString("pt-BR", {dateStyle:"short",timeStyle:"short"}) : "—"}</span></div>
            <div className="item-field"><b>livro</b><span>{item.livroId ? ((livroList.find(o => o.id === item.livroId) || {}).titulo ?? ("#" + item.livroId)) : "—"}</span></div>
            <div className="item-field"><b>leitor</b><span>{item.leitorId ? ((leitorList.find(o => o.id === item.leitorId) || {}).nome ?? ("#" + item.leitorId)) : "—"}</span></div>
            </div>
            <div className="item-actions">
      ...
      <label className="field-label">dataExpiracao</label>
      <input type="datetime-local" value={form["dataExpiracao"] ?? ""} onChange={e => setForm({...form, dataExpiracao: e.target.value})} />
      <label className="field-label">status</label>
...
export default function App() {
  ...
  const [aba, setAba] = React.useState("Livro");
  ...
  return (
    <div className="app-shell">
      <aside className="sidebar">
        <div className="sidebar-logo">Gestao Biblioteca<span className="dot">.</span></div>
        <button className={"nav-btn" + (aba === "Livro" ? " active" : "")} onClick={() => setAba("Livro")}><span className="nav-ico">🗂️</span>Livro</button>
        <button className={"nav-btn" + (aba === "Leitor" ? " active" : "")} onClick={() => setAba("Leitor")}><span className="nav-ico">🗂️</span>Leitor</button>
        <button className={"nav-btn" + (aba === "Emprestimo" ? " active" : "")} onClick={() => setAba("Emprestimo")}><span className="nav-ico">🗂️</span>Emprestimo</button>
      ...
      <main className="main">
        ...
        {aba === "Livro" && <PainelLivro token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Leitor" && <PainelLeitor token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
      {aba === "Emprestimo" && <PainelEmprestimo token={token} apiBaseUrl={apiBaseUrl} onPix={abrirPix} role={role} onLogout={sair} />}
